...
from toolkit.research_tool import run_research_tool
from toolkit.writer_tool import run_writer_tool

# Try to import MCP service - if not available, we'll just skip it
try:
    from infra.mcp.service import list_all_tools, run_mcp_tool
    MCP_AVAILABLE = True
except ImportError:
    MCP_AVAILABLE = False
    list_all_tools = None
...
def decide_agent(message: str, history: list) -> str:
    try:
        ...
        # Check for MCP tools
        if MCP_AVAILABLE and decision.startswith("MCP:"):
            return decision
